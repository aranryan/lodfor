...
## RevPAR short-term
```{r revpar_st, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_revpar_sa")
grtitle <- c("RevPAR, US")
footnote <- c("Source: STR; Tourism Economics")
...
## Occupancy, monthly simple forecast lower
```{r occ_monthly_simpf_lower, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
todo1 <- c("totus_occ_sa")
todo2 <- c("upmus_occ_sa")
todo3 <- c("midus_occ_sa")
todo4 <- c("ecous_occ_sa")
grtitle <- c("Occupancy, lower-priced chain scales")
footnote <- c("Source: STR; Tourism Economics")
starta <- as.Date("2010-01-01")
enda <- as.Date("2014-10-01")
variable_text <- c("Seasonally adjusted, monthly, \nIndex 2005=100")
variable_legend_1 <- c("US")
variable_legend_2 <- c("Upper midscale")
variable_legend_3 <- c("Midscale")
variable_legend_4 <- c("Economy")

temp <- outf_str_m_us_m %>% 
  filter(time >= starta, time <= enda, 
         variable == todo1 | variable == todo2 | variable == todo3 | 
           variable == todo4) %>%
  spread(variable, value) %>%
...
## Occupancy, monthly simple forecast higher
```{r occ_monthly_simpf_higher, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
todo1 <- c("totus_occ_sa")
todo2 <- c("luxus_occ_sa")
todo3 <- c("upuus_occ_sa")
todo4 <- c("upsus_occ_sa")
...
## ADR, monthly simple forecast
```{r adr_monthly_simpf_higher, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
todo1 <- c("totus_adr_sa")
#todo2 <- c("luxus_adr_sa")
todo3 <- c("upuus_adr_sa")
todo4 <- c("upsus_adr_sa")
grtitle <- c("ADR, higher-priced chain scales, \nmonthly")
footnote <- c("Source: STR; Tourism Economics")
starta <- as.Date("2010-01-01")
enda <- as.Date("2014-10-01")
variable_text <- c("Seasonally adjusted")
...
temp <- outf_str_m_us_m %>% 
  filter(time >= starta, time <= enda, 
         variable == todo1  | variable == todo3 | #| variable == todo2
           variable == todo4) %>%
  spread(variable, value) %>%
  melt(id.vars = c("time"))
...
holdnames <-setNames(c("grey70", mypallete[10:10], mypallete[8:8], mypallete[7:7]), 
                     c(todo1, todo3, todo4))
legendnames <- setNames(c(variable_legend_1, 
                          variable_legend_3, variable_legend_4),
                        c(todo1, todo3, todo4))
...
## ADR, monthly simple forecast lower
```{r adr_monthly_simpf_lower, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
todo1 <- c("totus_adr_sa")
todo2 <- c("upmus_adr_sa")
todo3 <- c("midus_adr_sa")
todo4 <- c("ecous_adr_sa")
grtitle <- c("ADR, lower-priced chain scales, \nmonthly")
footnote <- c("Source: STR; Tourism Economics")
starta <- as.Date("2013-01-01")
enda <- as.Date("2014-10-01")
variable_text <- c("Seasonally adjusted")
variable_legend_1 <- c("US")
variable_legend_2 <- c("Upper midscale")
