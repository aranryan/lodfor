...
footnote <- c("Note: Seasonally adjusted. GDP quarterly. Room demand monthly through January 2015.\nSource: STR, Tourism Economics")
...
todo <- c("totus_demd_sa")
grtitle <- c("US room demand and GDP")
footnote <- c("Note: Seasonally adjusted, quarterly. Historical room demand through fourth quarter 2014. \nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
...
temp <- usfor_q_m %>%
  filter(variable == todo, date >= starta, date <= enda) %>%
  index_m_melted(index_year=index_year) %>%
  mutate(variable=c("Room demand")) %>%
  mutate(forecast = cut(date, breaks = c(as.Date("1987-01-01"), as.Date("2014-10-01"), enda),
                          labels=c("History", "Forecast"),
                          right=TRUE)) 
temp2 <- usfor_q_m %>%
  filter(variable == "us_gdp") %>%
  index_q_melted(index_year=index_year) %>%
  mutate(variable=c("GDP")) %>%
  filter(date >= starta, date <= enda) %>%
  mutate(forecast = "History")
temp3 <- rbind(temp, temp2)

p1 <- ggplot(temp3, aes(x = date, y = value, color=variable)) +
  geom_line(aes(color=variable))  + 
  geom_point(aes(shape=forecast), show_guide=FALSE, cex=2) +
  scale_color_manual(values = c("grey40", mypallete[9:9])) +
  theme(legend.position=c(1, .2)) +
  scale_shape_manual(values=c(27,1)) +
  scale_x_date(breaks = "1 year", labels = date_format("%Y"))
plot_title_1(plot=p1, grtitle=grtitle, footnote=footnote)

a <- recordPlot()
replayPlot(a)
#print(plot_title_1(plot=p1, grtitle=grtitle, footnote=footnote))
# save image
...
footnote <- c("Note: Data through fourth quarter 2014. Household survey employment. \nSource: STR; Bureau of Labor Statistics; Tourism Economics")
starta <- as.Date("1987-01-01")
enda <- as.Date("2014-10-01")
...
p1 <- ggplot() +
 ...
 scale_x_date(breaks = "1 year", labels = date_format("%y")) +
 scale_color_manual(values = c(mypallete[10:10], "grey70")) +
 stat_smooth(data = temp, aes(x = date, y = value), method=lm, fill = "grey90", color="grey40", alpha=.4, span=1)
plot_title_1(plot=p1, grtitle=grtitle, footnote=footnote)
```
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  ...
  coord_cartesian(xlim = c(start_xaxis, end_xaxis)) +
  scale_y_continuous(labels=percent) +
  geom_line(color=mypallete[10:10], size=.8)+ 
  geom_hline(aes(yintercept=holdmean), color="grey40") +
  geom_point(data = temp[temp$date>=as.Date("2015-01-01"),], color=mypallete[10:10], pch=1, cex=3) +
...
temp <- ushist_m_m %>% 
  ...
  spread(variable, value) %>%
  melt(id.vars = c("date")) %>%
  mutate(group_date = cut(date, breaks = as.Date(c("2004-01-01", "2008-04-01", "2010-01-01","2015-04-01"))))
...
todo <- c("totus_adr_sa")
grtitle <- c("ADR, US")
footnote <- c("Note: Seasonally adjusted. History through fourth quarter of 2014. Forecast through fourth quarter of 2016.\nSource: STR; Tourism Economics")
starta <- as.Date("2004-01-01")
enda <- as.Date("2016-10-01")
variable_text <- c("")
...
p1 <- ggplot(data = temp, aes(x = date, y = value, shape = factor(group_date))) + 
 geom_line(data = temp[temp$date<=as.Date("2014-10-01"),], aes(x= date, y=value, group=1), size = .8, color=mypallete[10:10]) +
geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], aes(x= date, y=value, group=1), color=mypallete[10:10], size=.9, lty=2) +
 ggtitle(variable_text) + 
 scale_color_manual(values = c(mypallete[10:10], "grey70"))  + 
 stat_smooth(method="lm", fill = "grey70", 
             color="grey40", alpha=.4, span=.8, se = FALSE)+ 
...
todo <- c("totus_revpar_sarpc")
grtitle <- c("Real RevPAR, US")
footnote <- c("Note: Seasonally adjusted. History through the fourth quarter of 2014. Forecast through the fourth quarter of 2017.\nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("1987-01-01")
...
p1 <- ggplot() +
   geom_line(data = temp[temp$date<=as.Date("2014-10-01"),], 
             aes(x = date, y = value), color=mypallete[10:10], size=.7) + 
   geom_hline(yintercept = holdmean, colour = "grey50", size = 0.5) +
   geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], 
             aes(x = date, y = value), color="grey60", size=.7) +
 ggtitle(variable_text) + 
 scale_color_manual(values = c(mypallete[10:10], "grey70")) +
...
p1 <- ggplot() + 
 ...
 ggtitle(variable_text) + 
  scale_y_continuous(labels = dollar, limits=c(0,100)) +
   scale_x_date(labels = date_format("%y"), breaks = "1 year") +
...
footnote <- c("Note: Seasonally adjusted. Room demand quarterly through fourth quarter 2014.\nSource: STR, Tourism Economics")
...
temp <- temp %>%
  ...
  filter(variable != "topcoast_demd_sa") %>%
  filter(variable != "topcoast_wo_ny_demd_sa") 
...
temp <- temp %>%
  filter(date > "2005-01-01", date <= "2014-10-01")

levels(temp$variable)[levels(temp$variable)=="top25_demd_sa"] <- "Top 25 markets"
levels(temp$variable)[levels(temp$variable)=="topcoast_demd_sa"] <- "Selected coastal markets"
levels(temp$variable)[levels(temp$variable)=="topcoast_wo_nydc_demd_sa"] <- "Selected coastal markets w/o NY and DC"
...
footnote <- c("Note: Seasonally adjusted. Room supply quarterly through fourth quarter 2014.\nSource: STR, Tourism Economics")
variable_text <- c("Index (2006=100)")
index_year <- c(2006)
...
temp <- temp %>%
  ...
  filter(variable != "topcoast_supd_sa") %>%
  filter(variable != "topcoast_wo_ny_supd_sa") 
...
levels(temp$variable)[levels(temp$variable)=="top25_supd_sa"] <- "Top 25 markets"
levels(temp$variable)[levels(temp$variable)=="topcoast_supd_sa"] <- "Selected coastal markets"
levels(temp$variable)[levels(temp$variable)=="topcoast_wo_nydc_supd_sa"] <- "Selected coastal markets w/o NY and DC"
...
## Occupancy, lower priced
```{r chain_occ_lower_priced}
# grtitle <- c("Occupancy, lower priced")
# footnote <- c("Note: Seasonally adjusted. Room supply quarterly through fourth quarter 2014.\nSource: STR, Tourism Economics")
# variable_text <- c("Index (2006=100)")
# index_year <- c(2006)
# 
...
b <- usfor_q$us_pc
c <- usfor_q$totus_revpar_sa
index_year <- 2014
  start_index <- as.Date(paste(index_year,"-01-01",sep=""))
  end_index <- as.Date(paste(index_year,"-10-01",sep=""))
start_index
end_index
  temp_mean <- mean(window(usfor_q$us_pc, start=start_index, end=end_index))
temp_mean
