...
knitr::opts_chunk$set(cache=TRUE, cache.path='output_data/cache/', autodep=TRUE)
fpath_graphs <- c("output_data/us_overviewgraphs/")
```
...
# png(filename=paste(fpath_graphs,"demand_monthly_gdp.png",sep=""), 
#     type="cairo",
#     units="in", 
#     width=9, 
#     height=5.7, 
...
```{r demand_gdp_forecast, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_demd_sa")
grtitle <- c("US room demand and GDP")
footnote <- c("Note: Historical room demand through fourth quarter 2014 \nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
enda <- as.Date("2016-10-01")
index_year <- as.numeric(2006)
variable_text <- c(paste("Demand and GDP quarterly,\nseasonally adjusted,\nindex ",index_year,"=100",sep=""))
variable_legend_1 <- c("Room demand")
...
p1 <- ggplot() +
  ...
  geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], aes(x = date, y = value), color=mypallete[10:10], size=.9, lty=2) + 
  geom_line(data = temp2, aes(x = date, y = value, color=variable), size = .9) +
  ggtitle(variable_text) +
  theme(legend.position=c(1, .2)) +
...
```{r occupancy_forecast, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_occ_sa")
grtitle <- c("Occupancy")

footnote <- c("Note: History through fourth quarter 2014, forecast through fourth quarter 2017\nSource: STR; Tourism Economics")
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  ...
  coord_cartesian(xlim = c(start_xaxis, end_xaxis)) +
  scale_y_continuous(labels=percent) +
  geom_line(data = temp[temp$date<=as.Date("2014-10-01"),], color=mypallete[10:10]) + 
  geom_hline(aes(yintercept=holdmean), color="grey40") +
  geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], color=mypallete[10:10], size=.8, lty=2) +
  annotate("text", x = as.Date("1998-01-01"), y = holdmean+.008, size=3,
...
```{r adr_st_forecast, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
todo <- c("totus_adr_sa")
grtitle <- c("ADR, US")
footnote <- c("Source: STR; Tourism Economics")
starta <- as.Date("2004-01-01")
...
```{r revpar_real_lt_fcst, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_revpar_sarpc")
grtitle <- c("Real RevPAR, US")
footnote <- c("Source: STR; Bureau of Economic Analysis; Tourism Economics")
...
p1 <- ggplot() +
   ...
   geom_hline(yintercept = holdmean, colour = "grey50", size = 0.5) +
   geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], 
             aes(x = date, y = value), color=mypallete[10:10], size=.8, lty=2) +
 ggtitle(variable_text) + 
 scale_color_manual(values = c(mypallete[10:10], "grey70")) +
 annotate("text", x = as.Date("1988-01-01"), y = holdmean+1.5, size=3,
         label = c(paste("Long-term average:\n$",sprintf("%.2f", holdmean), " (2000 to 2014)",sep="")))+ 
