...
starta <- as.Date("2004-01-01")
endhistory <- as.Date("2014-10-01")
startforecast <- as.Date("2014-10-01")
...
p1 <- ggplot(temp4, aes(x = date, y = value, color=variable)) +
  geom_line(size=.8)  +
  scale_color_manual(values = setNames(c(mypallete[9:9], "grey40"), c(todo, "us_gdp")), 
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  ...
  geom_line(data = temp[temp$date>=as.Date("2015-01-01"),], color=mypallete[10:10], size=.8, linetype="99") +
...
p1 <- ggplot() +
   ...
   geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], 
             aes(x = date, y = value), color=mypallete[10:10], size=.7, linetype = "99") +
   geom_hline(yintercept = holdmean, colour = "grey50", size = 0.5) + 
   ggtitle(variable_text) + 
   scale_color_manual(values = c(mypallete[10:10], "grey70")) +
