...
outf_str_us_m <- df1
outf_str_us_m <- melt(outf_str_us_m, id.vars = c("time"))
...
temp <- ushist_m_m %>%
  filter(variable == todo, time >= starta, time <= enda) %>%
  index_m_melted(index_year=index_year)
temp2 <- ushist_q_m %>%
  filter(variable == "us_gdp", time >= starta, time <= enda) %>%
