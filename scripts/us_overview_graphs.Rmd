...
temp <- arrange(temp, seg)
# to assign colors by seg
cols <- c("grey60", mypallete[10:10])
names(cols) <- c(todo[2], todo2[2])
...
temp <- temp %>%
  ...
  filter(variable != "topcoastwony_demd_sa") %>%
  filter(variable != "cawawosf_demd_sa")
...
temp <- temp %>%
  ...
  filter(variable != "topcoastwony_supd_sa") %>%
  filter(variable != "cawawosf_supd_sa")
...
# because this doesn't depend on top25compile_td it is commented out and 
# temp1 is set to Null
...
temp2 <- ushist_q_td %>%
  filter(seg == "sanfrancisco" | seg == "totus") %>%
  filter(date >= "1987-01-01") %>%
  select_("date", "seg", todo)
...
levels(temp$seg)[levels(temp$seg)=="totus"] <- "Total US"
...
p1 <- ggplot() +
  ...
  theme(legend.position=c(1, .15)) +
    scale_x_date(breaks = "1 year", labels = date_format("%Y")) +
  scale_color_manual(values = c(mypallete[9:9], "grey60")) +
...
footnote <- c("Note: Seasonally adjusted. Occupancy quarterly through fourth quarter 2014.\nSource: STR, Tourism Economics")
