...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, fig.path='../output_data/figure_us_overview_graphs/fig-', dev="CairoPNG", dev.args = list(res = 800) )
```
...
todo <- c("totus_demd_sa")
grtitle <- c("US room demand and GDP")
footnote <- c("Note: Seasonally adjusted. Historical room demand through fourth quarter 2014. \nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
...
p1 <- ggplot() +
  ...
  geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], aes(x = date, y = value), color=mypallete[9:9], size=.9, lty=2) + 
  geom_line(data = temp2, aes(x = date, y = value, color=variable), size = .9) +
  ggtitle(variable_text) +
  theme(legend.position=c(1, .2)) +
