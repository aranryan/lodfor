...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE, cache.path='../output_data/cache/', autodep=TRUE, fig.path='../output_data/figure_us_overview_graphs/fig-', dev='png', out.width='9in', dpi=400)
```
...
footnote <- c("Note: Seasonally adjusted. GDP quarterly. Room demand monthly through December 2014.\nSource: STR, Tourism Economics")
...
p1 <- ggplot() +
  ...
  geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], aes(x = date, y = value), color=mypallete[9:9], size=.9, lty=4) + 
  geom_line(data = temp2, aes(x = date, y = value, color=variable), size = .9) +
  ggtitle(variable_text) +
  theme(legend.position=c(1, .2)) +
...
todo <- c("totus_revpar_sarpc")
grtitle <- c("Real RevPAR, US")
footnote <- c("Note: Seasonally adjusted./nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("1987-01-01")
