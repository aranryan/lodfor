...
# when kniting from the button, I had to add "../" in front of each
load(paste("output_data/ushist_q.Rdata", sep=""))
load(paste("output_data/ushist_m.Rdata", sep=""))
load(paste("output_data/outf_str_us_m.Rdata", sep=""))
```
...
fname <- c(paste("input_data/code_book_matrix.csv", sep=""))
=======
ushist_m_m <- ushist_m %>%
...
## Occupancy 
```{r occupancy, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_occ_sa")
grtitle <- c("Occupancy")
...
start_xaxis <- as.Date("2004-01-01") # for x-axis
#end_xaxis <- as.Date("2013-12-01") # for x-axis
...
p1 <- ggplot() +
  geom_line(data = temp, aes(x = date, y = value, color=measure), size = .7) +
  scale_color_manual(values = c(mypallete[10:10], "grey70")) +
  coord_cartesian(xlim = c(start_xaxis, end_xaxis)) +
  ggtitle(variable_text)  
#direct.label(p1,"lines2")
plot_title_1(plot=p1, grtitle=grtitle, footnote=footnote)

p <- ggplot(data = temp, aes(x = date, y=value, color = measure)) +
...
## ADR growth 
```{r adr_growth, echo=FALSE, message=FALSE, warning=FALSE, dev='CairoPNG'}
#sets variable and text items
todo <- c("totus_adr_sa")
grtitle <- c("ADR")
