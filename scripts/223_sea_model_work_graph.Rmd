...
### Evoluation of estimates
```{r}

view_ser <- out_hold %>%
  filter(model_id == "model_8") %>%
  filter(date >= as.Date("2015-01-01") & date <= as.Date("2015-12-01")) %>%
  filter(train_end >= as.Date("2013-12-01") & train_end <= as.Date("2014-12-01"))
...
p1 <- ggplot(data=view_ser)
p1 <- p1 + geom_line(aes(x=date, y=ogstl_fct_pchya, color=factor(train_end)))
p1 <- p1 + geom_line(aes(x=date, y=og_act_pchya, color=factor(train_end)))
p1
