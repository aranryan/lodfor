...
```{r readchunk}
#read_chunk('~/Project/R projects/lodfor/scripts/functions_combined.R')
#source('~/Project/R projects/lodfor/scripts/setup.R')
...
# df1 <- data.frame(date=time(xts1), xts1)
# outf_str_us_m <- df1
# outf_str_us_m <- melt(outf_str_us_m, id.vars = c("date"))
...
# xts1 <- ushist_m
# xts1 <- window(xts1, end = as.Date("2014-12-01"))
# df1 <- data.frame(date=time(xts1), xts1)
# ushist_m <- df1
# ushist_m_m <- melt(ushist_m, id.vars = c("date"))
# #ushist_m_m[1:4,]
# 
...
# df1 <- data.frame(date=time(xts1), xts1)
# ushist_q_df <- df1
# ushist_q_m <- melt(ushist_q_df, id.vars = c("date"))
# #ushist_q_m[1:4,]
# 
# # this is if I wanted to do things with a tidy format
# tidy_ushist_q <- data.frame(date=time(ushist_q), ushist_q)%>%
...
```{r create_forecastdataframe, echo=FALSE, message=FALSE, warning=FALSE}
...
footnote <- c("Note: Seasonally adjusted. GDP quarterly. Room demand monthly through December 2015.\nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
enda <- as.Date("2016-01-01")
index_year <- as.numeric(2005)
...
p1 <- ggplot() +
  ...
  stat_smooth(data = temp, aes(x = date, y = value), method="loess", 
              color=mypallete[9:9], alpha=.4, span=.2, se = FALSE, fullrange=FALSE, size=.9, show.legend=FALSE) +
...
todo <- c("totus_demd_sa")
grtitle <- c("US room demand and GDP")
footnote <- c("Note: Seasonally adjusted, quarterly. Historical room demand through fourth quarter 2015. \nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
endhistory <- as.Date("2015-12-01")
startforecast <- as.Date("2015-10-01")
...
```{r demand_ratio_pop_fcst}
#sets variable and text items
#todo <- c("totus_demd_sa")
grtitle <- c("Room demand per capita")
footnote <- c("Note: Historical trend line fitted from 1987 to 2007Q4.\nSource: STR; Census Bureau; Tourism Economics")
starta <- as.Date("1987-01-01")
...
# adds a column based on date to be the subset pre recession
temp_a <- temp %>%
  mutate(hist = ifelse(date <= as.Date("2007-10-01"), 1, 0))
# fits model based on that subset
temp_b <- temp_a %>%
  filter(hist == 1)
fm <- lm(value ~ date, temp_b)
# fm <- lm(value ~ date, temp_b, subset = temp$hist == 1)
# predicts values
b <- predict(fm, temp_a)
# binds the values onto the dataframe and renames
temp_a <- cbind(temp_a, b) %>%
  rename(fitted = b)

coefficients(fm)

p1 <- ggplot() +
  geom_line(data = temp_a, aes(x = date, y = value, color = variable), size = .7) +
 ggtitle(variable_text) + 
  scale_color_manual(values = c(mypallete[10:10], "grey70")) +
  # subset to only plot line when hist equals 1
 geom_line(data = subset(temp_a, hist == 1), aes(x = date, y = fitted, color = "grey70"), size = .9) +
 geom_line(data = subset(temp_a, hist == 0), aes(x = date, y = fitted, color = "grey70"), size = .9, linetype="99") 
...
```{r demand_ratio_pop_lnfcst}
#sets variable and text items
#todo <- c("totus_demd_sa")
grtitle <- c("Room demand per capita")
footnote <- c("Note: Historical trend line fitted from 1987Q1 to 2007Q4 in log terms.\nSource: STR; Census Bureau; Tourism Economics")
starta <- as.Date("1987-01-01")
enda <- as.Date("2017-10-01")
variable_text <- c("Annual room nights per capita \n(working age population)")
variable_legend_1 <- c("Room demand")
...
temp <- usfor_q_m %>% 
  filter(variable == "totus_demd_sa" | variable == "us_popw", date >= starta, date <= enda) %>%
  spread(variable, value) %>%
  mutate(dem_popw = log(totus_demd_sa * 365 / us_popw*1000)) %>%
  select(date, dem_popw) %>%
  gather(variable, value, -date)
...
temp_a <- cbind(temp_a, b) %>%
  rename(fitted = b)
temp_a <- temp_a %>%
  mutate(value = exp(value), fitted = exp(fitted))
...
p2 <- p1 +
geom_rect(data=subset(recession_df_m, start_rec >= min(temp_a$date)), #trims length
            aes(xmin=start_rec
                ,xmax=end_rec
                ,ymin=-Inf  # to bottom of graph
                ,ymax=Inf), # to top of graph
...
p2
plot_title_3(plot=p2, grtitle=grtitle, footnote=footnote, filename = c("temp.png"))
...
footnote <- c("Note: Data through fourth quarter 2015. Household survey employment. \nSource: STR; Bureau of Labor Statistics; Tourism Economics")
starta <- as.Date("1987-01-01")
enda <- as.Date("2015-12-01")
...
# this direct label stopped working in Sep 2015
# p1 <- direct.label (p1,
...
footnote <- c("Note: Seasonally adjusted. History through fourth quarter 2015, forecast through fourth quarter 2017.\nSource: STR; Tourism Economics")
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  geom_line(data = temp[temp$date<=as.Date("2015-12-01"),], color=mypallete[10:10], size=.8) + 
  geom_line(data = temp[temp$date>=as.Date("2015-10-01"),], color=mypallete[10:10], size=.8, linetype="99") +
...
temp <- ushist_q_m %>% 
  ...
  spread(variable, value) %>%
  gather(variable, value, -date)%>%
  mutate(group_date = cut(date, breaks = as.Date(c("1987-01-01", "1998-01-01","2015-12-01"))))
...
todo <- c("totus_adr_sarpc")
grtitle <- c("Real ADR, US")
footnote <- c("Note: Seasonally adjusted.Historical data through fourth quarter of 2015. \nSource: STR; Bureau of Economic Analysis; Tourism Economics")
...
temp <- ushist_q_m %>% 
  ...
  spread(variable, value) %>%
  gather(variable, value, -date) %>%
  mutate(group_date = cut(date, breaks = as.Date(c("2004-01-01", "2008-04-01", "2010-01-01","2015-12-01"))))
...
todo <- c("totus_adr_sa")
grtitle <- c("ADR, US")
footnote <- c("Note: Seasonally adjusted quarterly data.\nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("2006-01-01")
enda <- as.Date("2015-12-01")
variable_text <- c("")
...
footnote <- c("Note: Seasonally adjusted.Historical data through December 2015. \nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("2006-01-01")
enda <- as.Date("2015-12-01")
variable_text <- c("")
...
todo <- c("totus_adr_sa")
grtitle <- c("ADR, US")
footnote <- c("Note: Seasonally adjusted. History through fourth quarter of 2015. Forecast through fourth quarter of 2016.\nSource: STR; Tourism Economics")
starta <- as.Date("2004-01-01")
enda <- as.Date("2016-10-01")
variable_text <- c("")
...
p1 <- ggplot(data = temp, aes(x = date, y = value, shape = factor(group_date))) + 
 geom_line(data = temp[temp$date<=as.Date("2015-04-01"),], aes(x= date, y=value, group=1), size = .8, color=mypallete[10:10]) +
...
```{r adr_gr_st}
n#sets variable and text items
...
temp <- ushist_q_m %>% 
  ...
  select(-value) %>%
 # filter(date >= starta, date <= enda) %>%
 # reshape2::melt(id.vars = c("date", "group_date")) %>%
  gather(variable, value, -date, -group_date) %>%
...
# doesn't add minor tick marks, just for grid I guess 
 scale_x_date(labels = date_format("%Y"), date_breaks = "1 year", date_minor_breaks = "3 months") 
# useful
...
todo <- c("totus_revpar_sarpc")
grtitle <- c("Real RevPAR, US")
footnote <- c("Note: Seasonally adjusted, 2014 dollars. History through the third quarter of 2015. Forecast through the fourth quarter of 2017.\nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("1987-01-01")
enda <- as.Date("2017-10-01")
# most recent historical quarter
endq <- as.Date("2015-10-01")
variable_text <- c("")
...
holdcurrent <- subset(temp, date == endq)
...
p1 <- ggplot() +
   geom_line(data = temp[temp$date<=endq,], 
             aes(x = date, y = value), color=mypallete[10:10], size=.9) + 
   geom_line(data = temp[temp$date>=endq,], 
             aes(x = date, y = value), color=mypallete[10:10], size=.9, linetype = "99") +
   ...
   annotate("text", x = as.Date("1988-01-01"), y = holdmean+1.9, size=3,
         label = c(paste("Long-term average:\n$",sprintf("%.0f", holdmean), " (2000 to 2014)",sep="")))+ 
   scale_y_continuous(labels = dollar, limits=c(40,85)) + 
   geom_point(data = temp[temp$date==endq,], 
             aes(x = date, y = value), colour=mypallete[10:10], shape=16, size=3) +  # this adds a red point
   geom_text(data = temp[temp$date==endq,], 
             aes(x = date, y = value), label=paste(holdcurrent_date, " $", 
                                          holdcurrent_ob, sep=""), 
...
p2 <- p1 +
  ...
  annotate("text", x = as.Date("2007-07-01"), y = holdhigh+2, size=3,
...
p1 <- ggplot() + 
  geom_line(data = temp[temp$date<=as.Date("2015-12-01"),], 
            aes(x = date, y = value), color=mypallete[10:10], size=.9) + 
  geom_line(data = temp[temp$date>=as.Date("2015-10-01"),], 
            aes(x = date, y = value), color=mypallete[10:10],  size=.9, linetype = "99") +
#   geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], 
#             aes(x = date, y = value), color=mypallete[10:10], size=.5) +
#     geom_point(data = temp[temp$date>=as.Date("2014-10-01"),], 
...
footnote <- c("Note: Seasonally adjusted. Room demand quarterly through fourth quarter 2015.\nSource: STR, Tourism Economics")
...
temp <- temp %>%
  filter(date > "2005-01-01", date <= "2015-12-01")
...
footnote <- c("Note: Seasonally adjusted. Room supply quarterly through fourth quarter 2015.\nSource: STR, Tourism Economics")
variable_text <- c("Index (2006=100)")
index_year <- c(2006)
...
temp <- temp %>%
  filter(date > "1995-01-01", date <= "2015-12-01")
...
footnote <- c("Note: Seasonally adjusted data quarterly through fourth quarter 2015.\nSource: STR, Tourism Economics")
variable_text <- c("Index (2006=100)")
index_year <- c(2006)
todo <- c("supd_sa")
...
plot_title_3(plot=p3, grtitle=grtitle, footnote=footnote, filename = c("temp.png"))
...
p1 <- ggplot(data = temp, aes(x = date, y = value, shape = factor(group_date))) + 
  geom_line(aes(x= date, y=value, group=1), size = .7, color=mypallete[10:10]) +
  ggtitle(variable_text) + 
  scale_y_continuous(labels = dollar, limits=c(90,max(temp$value, na.rm=TRUE) + 5)) +
  geom_point(data=holdcurrent, colour=mypallete[10:10], shape=16, size=3) +  # this adds a red point
  geom_text(data=holdcurrent, label=paste(holdcurrent_date, " $", 
                                          holdcurrent_ob, sep=""), 
            vjust=0, hjust=1.2, size=3) # this adds a label for the red point
...
p2 <- p1 +
  geom_hline(aes(yintercept=holdmean), color="grey60", size=.5, alpha=.6) +
  annotate("text", x = as.Date("2004-01-01"), y = holdmean+2, size=3,
           label = c(paste("Long-term average: $",sprintf("%0.f", holdmean), "\n(2000 to 2014)",sep="")))
# add high
p2 <- p2 +
  geom_hline(aes(yintercept=holdhigh), color="grey60", size=.5, alpha=.6) +
...
## ADR, real short-term
```{r upm_adr_real_st}
todo <- c("upmus_adr_sarpc")
grtitle <- c("Real ADR, Upper midscale")
footnote <- c("Note: Seasonally adjusted.\nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("1995-01-01")
enda <- as.Date("2015-12-01")
variable_text <- c("In 2014 dollars")
...
p1 <- ggplot(data = temp, aes(x = date, y = value, shape = factor(group_date))) + 
  geom_line(aes(x= date, y=value, group=1), size = .7, color=mypallete[10:10]) +
  ggtitle(variable_text) + 
  scale_y_continuous(labels = dollar, limits=c(80,max(temp$value, na.rm=TRUE) + 5)) +
  geom_point(data=holdcurrent, colour=mypallete[10:10], shape=16, size=3) +  # this adds a red point
  geom_text(data=holdcurrent, label=paste(holdcurrent_date, " $", 
                                          holdcurrent_ob, sep=""), 
            vjust=0, hjust=1.2, size=3) # this adds a label for the red point
