...
footnote <- c("Note: Seasonally adjusted. GDP quarterly. Room demand monthly through September 2015.\nSource: STR, Tourism Economics")
...
todo <- c("totus_demd_sa")
grtitle <- c("US room demand and GDP")
footnote <- c("Note: Seasonally adjusted, quarterly. Historical room demand through third quarter 2015. \nSource: STR, Tourism Economics")
starta <- as.Date("2004-01-01")
endhistory <- as.Date("2015-09-01")
startforecast <- as.Date("2015-09-01")
...
footnote <- c("Note: Data through third quarter 2015. Household survey employment. \nSource: STR; Bureau of Labor Statistics; Tourism Economics")
starta <- as.Date("1987-01-01")
...
footnote <- c("Note: Seasonally adjusted. History through third quarter 2015, forecast through fourth quarter 2017.\nSource: STR; Tourism Economics")
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  geom_line(data = temp[temp$date<=as.Date("2015-07-01"),], color=mypallete[10:10], size=.8) + 
  geom_line(data = temp[temp$date>=as.Date("2015-07-01"),], color=mypallete[10:10], size=.8, linetype="99") +
...
temp <- ushist_q_m %>% 
  ...
  spread(variable, value) %>%
  gather(variable, value, -date)%>%
  mutate(group_date = cut(date, breaks = as.Date(c("1987-01-01", "1998-01-01","2015-10-01"))))
...
todo <- c("totus_adr_sarpc")
grtitle <- c("Real ADR, US")
footnote <- c("Note: Seasonally adjusted.Historical data through third quarter of 2015. \nSource: STR; Bureau of Economic Analysis; Tourism Economics")
...
temp <- ushist_q_m %>% 
  ...
  spread(variable, value) %>%
  gather(variable, value, -date)%>%
  mutate(group_date = cut(date, breaks = as.Date(c("2004-01-01", "2008-04-01", "2010-01-01","2015-11-01"))))
...
footnote <- c("Note: Seasonally adjusted.Historical data through August 2015. \nSource: STR; Bureau of Economic Analysis; Tourism Economics")
starta <- as.Date("2006-01-01")
enda <- as.Date("2015-11-01")
variable_text <- c("")
...
todo <- c("totus_adr_sa")
grtitle <- c("ADR, US")
footnote <- c("Note: Seasonally adjusted. History through third quarter of 2015. Forecast through fourth quarter of 2016.\nSource: STR; Tourism Economics")
starta <- as.Date("2004-01-01")
enda <- as.Date("2016-10-01")
variable_text <- c("")
...
p1 <- ggplot() + 
  geom_line(data = temp[temp$date<=as.Date("2015-07-01"),], 
            aes(x = date, y = value), color=mypallete[10:10], size=.9) + 
    geom_line(data = temp[temp$date>=as.Date("2015-07-01"),], 
            aes(x = date, y = value), color=mypallete[10:10],  size=.9, linetype = "99") +
#   geom_line(data = temp[temp$date>=as.Date("2014-10-01"),], 
...
footnote <- c("Note: Seasonally adjusted. Room demand quarterly through third quarter 2015.\nSource: STR, Tourism Economics")
...
footnote <- c("Note: Seasonally adjusted. Room supply quarterly through third quarter 2015.\nSource: STR, Tourism Economics")
variable_text <- c("Index (2006=100)")
index_year <- c(2006)
...
footnote <- c("Note: Seasonally adjusted data quarterly through third quarter 2015.\nSource: STR, Tourism Economics")
variable_text <- c("Index (2006=100)")
index_year <- c(2006)
todo <- c("supd_sa")
