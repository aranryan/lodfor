...
starta <- as.Date("2004-01-01")
endhistory <- as.Date("2015-04-01")
startforecast <- as.Date("2015-04-01")
...
p1 <- ggplot(temp, aes(x = date, y=value)) +
  geom_line(data = temp[temp$date<=as.Date("2015-04-01"),], color=mypallete[10:10], size=.8) + 
...
p2 <- p2 +
  ...
  annotate("text", x = as.Date("2007-07-01"), y = holdhigh+2, size=3,
           label = c(paste("Previous peak: $",sprintf("%0.f", holdhigh), "\n(2007 Q3)",sep="")))
plot_title_3(plot=p2, grtitle=grtitle, footnote=footnote, filename = c("temp.png")NN)
