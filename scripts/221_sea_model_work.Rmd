...
df_1 <- data_frame(start_1 = as.Date("1987-01-01"), end_1 = seq(from =2008, to = 2015), hold = list(x2,x2,x2,x2,x2,x2,x2,x2))

df_2 <- df_1 %>%
  # formats date
  mutate(end_1 = as.Date(paste0(end_1, "-01-01"))) %>%
  # uses the function I defined to filter based on start and end dates
  mutate(hold_2 = pmap(list(x=hold, start=start_1, end=end_1), f_temp1)) %>% 
  # converts to time series
  mutate(hold_ts = map(hold_2, f_convert_to_ts))


# for testing as I built
a <- df_3$hold_df3[[2]]
str(a)
