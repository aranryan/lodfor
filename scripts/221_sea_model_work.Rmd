...
#extract series
df_4 <- df_3 %>%
  mutate(selected_ser = map(model, f_extract_series)) %>%
  mutate(selected_ser2 = pmap(list(x=selected_ser, start_var=start_1, end_var=end_1), f_ready_1))

# temporary look at output
#a_1 <- df_4$selected2[[1]]

est_factors_1 <- df_4 %>%
  select(selected_ser2) %>%
  unnest() 

# join on the original data
